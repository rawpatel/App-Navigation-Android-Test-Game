...
import com.appnavigation.databinding.FragmentGameBinding
...
class GameFragment : Fragment() {
    ...
    // resources so we could internationalize, (or better yet, not define the question in code.)

    private val questions: MutableList<Question> = mutableListOf(
        Question(text = "What is Android Jetpack?",
            answers = listOf("all of these", "tools", "documentation", "libraries")),
        Question(text = "Base class for Layput?",
            answers = listOf("ViewGroup", "ViewSet", "ViewCollection", "ViewRoot")),
        Question(text = "Layout for Complex Screen?",
    ...
    override fun onCreateView(
        ...
        // Set the onClickLister for the submitButton
        binding.submitButton.setOnClickListener { view: View ->
            val checkedId = binding.questionRadioGroup.checkedRadioButtonId
            // DO noting if noting is checked (id == -1)
            if (-1 != checkedId){
                var answerIndex = 0
            when (checkedId) {
            ...
            if (answers[answerIndex] == currentQuestion.answers[0]){
                ...
                // Game Over! A worng answer sends us to the gameOverFragment.
